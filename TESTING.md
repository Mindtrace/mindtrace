...
- MinIO (ports in [`mindtrace/core/mindtrace/core/testing/minio.env`](./mindtrace/core/mindtrace/core/testing/minio.env) — host API/console mapped onto in-container MinIO defaults. Start via `scripts/docker_up.sh`, which passes that file as Compose `--env-file`. Do not invoke `tests/docker-compose.yml` without it.)
- Redis
- RabbitMQ
- MongoDB
