...
async def async_example():
    ...
    connection_manager = EchoService.launch(port=8081, host="localhost", wait_for_launch=True, timeout=30)
    ...
    try:
        ...
        tasks = [connection_manager.aecho(message=f"Concurrent message {i}") for i in range(1, 4)]
        ...
        job_result = await connection_manager.aecho(message="Async raw output call", validate_output=False)
...
def connection_manager_demo():
    ...
    methods = [
        attr
        for attr in dir(EchoConnectionManager)
    ...
    with EchoService.launch(port=8080, wait_for_launch=True, timeout=15) as cm:
        ...
        assert status_result.status == ServerStatus.AVAILABLE, (
            f"Expected status to be available, got {status_result.status}"
        )
