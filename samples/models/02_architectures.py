...
m = build_model("resnet50", "mlp", num_classes=10, pretrained=False, hidden_dim=512, dropout=0.2)
print(f"  head: {type(m.head).__name__}")
out = m(x)
print(f"  output shape: {tuple(out.shape)}")  # (2, 10)
...
m_frozen = build_model("resnet50", "linear", num_classes=3, pretrained=False, freeze_backbone=True)
