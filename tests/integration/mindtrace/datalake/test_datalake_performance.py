...
class TestDatalakePerformance:
    ...
    async def test_large_data_registry_performance(self, datalake: Datalake, temp_registry_dir: str):
        ...
        for size in sizes:
            large_data = {
                "large_string": "x" * size,
                "metadata": {"size": size, "test": "large_data_performance"}
            }
            
            start_time = time.time()
            datum = await datalake.add_datum(
                large_data, 
                {"size": size, "test": "performance"}, 
        ...
        for i in range(num_operations):
            if i % 2 == 0:
                ...
                data = {
                    "type": "registry", 
                    "index": i, 
                    "content": f"registry_content_{i}" * 100
                }
    ...
    async def test_error_recovery_performance(self, datalake: Datalake):
        ...
        with pytest.raises(DocumentNotFoundError):
            nonexistent_tasks = [datalake.get_datum(PydanticObjectId(f"0123456789abcdef012345{i}{j}")) for i in range(10) for j in range(10)]
