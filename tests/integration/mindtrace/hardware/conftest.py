...
async def real_cameras(api_client) -> List[str]:
    ...
    real_cameras = [camera for camera in all_cameras if not any(mock_term in camera for mock_term in ["Mock", "mock"])]
...
async def real_backends(api_client) -> List[str]:
    ...
    real_backends = [backend for backend in all_backends if not backend.startswith("Mock")]
