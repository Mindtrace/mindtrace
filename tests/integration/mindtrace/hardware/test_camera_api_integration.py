...
async def test_camera_discovery_and_connection(api_client: httpx.AsyncClient, real_cameras: List[str]):
    ...
    # Initialize camera
    response = await api_client.post(f"/cameras/{test_camera}/initialize", json={"test_connection": True})
    assert response.status_code == 200
...
@pytest.mark.slow
@pytest.mark.asyncio
async def test_complete_camera_workflow(api_client: httpx.AsyncClient, initialized_camera: str, temp_dir: str):
    ...
    response = await api_client.put(f"/cameras/{camera}/exposure", json={"exposure": mid_exposure})
    assert response.status_code == 200
    ...
    response = await api_client.put(f"/cameras/{camera}/gain", json={"gain": mid_gain})
    assert response.status_code == 200
    ...
    response = await api_client.post(
        f"/cameras/{camera}/capture", json={"return_image": True, "save_path": capture_path}
    )
    assert response.status_code == 200
    ...
    # 5. Test HDR capture
    response = await api_client.post(
        f"/cameras/{camera}/capture/hdr", json={"exposure_levels": 3, "exposure_multiplier": 2.0, "return_images": True}
    )
    assert response.status_code == 200
    ...
    response = await api_client.post(f"/cameras/{camera}/config/export", json={"config_path": config_path})
    assert response.status_code == 200
    ...
    # Modify settings
    response = await api_client.put(f"/cameras/{camera}/exposure", json={"exposure": exposure_range[0]})
    assert response.status_code == 200

    # Import configuration (should restore previous settings)
    response = await api_client.post(f"/cameras/{camera}/config/import", json={"config_path": config_path})
    assert response.status_code == 200
...
@pytest.mark.slow
@pytest.mark.asyncio
async def test_network_bandwidth_management(api_client: httpx.AsyncClient, real_cameras: List[str], cleanup_cameras):
    ...
    try:
        ...
        # Set new limit
        new_limit = 3
        response = await api_client.put("/network/concurrent-limit", json={"limit": new_limit})
        assert response.status_code == 200
        ...
        await api_client.put("/network/concurrent-limit", json={"limit": original_limit})
...
@pytest.mark.slow
@pytest.mark.asyncio
async def test_batch_operations(api_client: httpx.AsyncClient, real_cameras: List[str], cleanup_cameras):
    ...
    # Batch initialize
    response = await api_client.post(
        "/cameras/batch/initialize", json={"cameras": test_cameras, "test_connections": True}
    )
    assert response.status_code == 200
    ...
    try:
        ...
        response = await api_client.post(
            "/cameras/batch/capture", json={"cameras": test_cameras, "return_images": False}
        )
        assert response.status_code == 200
        ...
        # Batch HDR capture
        response = await api_client.post(
            "/cameras/batch/capture/hdr", json={"cameras": test_cameras, "exposure_levels": 2, "return_images": False}
        )
        assert response.status_code == 200
...
@pytest.mark.slow
@pytest.mark.asyncio
async def test_video_streaming(api_client: httpx.AsyncClient, initialized_camera: str):
...
@pytest.mark.slow
@pytest.mark.asyncio
async def test_error_handling_and_edge_cases(api_client: httpx.AsyncClient, real_cameras: List[str]):
    ...
    try:
        ...
        response = await api_client.put(
            f"/cameras/{real_camera}/exposure",
            json={
...
@pytest.mark.slow
@pytest.mark.asyncio
async def test_camera_state_management(api_client: httpx.AsyncClient, real_cameras: List[str], cleanup_cameras):
