"""Integration tests for race conditions in MinioRegistryBackend lock acquisition.

These tests verify that the lock acquisition mechanism correctly handles
race conditions in a distributed environment.
"""
...
def test_concurrent_lock_acquisition_race_condition(minio_backend, minio_client, minio_test_bucket):
    ...
    try:
        minio_client.remove_object(minio_test_bucket, f"_lock_{lock_key}")
    except Exception:
        pass
    ...
    def acquire_lock_thread(thread_id: int):
        ...
        try:
            result = minio_backend.acquire_lock(lock_key, lock_id, timeout, shared=False)
            with results_lock:
    ...
    is_locked, lock_id = minio_backend.check_lock(lock_key)
    ...
    # Clean up
    minio_backend.release_lock(lock_key, successful_acquires[0][2])


def test_concurrent_shared_lock_acquisition(minio_backend, minio_client, minio_test_bucket):
    ...
    def acquire_shared_lock_thread(thread_id: int):
        ...
        try:
            result = minio_backend.acquire_lock(lock_key, lock_id, timeout, shared=True)
            with results_lock:
                results.append((thread_id, result, lock_id))
            # Hold lock briefly
            time.sleep(0.1)
...
def test_exclusive_lock_conflicts_with_shared_locks(minio_backend, minio_client, minio_test_bucket):
    ...
    shared_result = minio_backend.acquire_lock(lock_key, shared_lock_id, timeout, shared=True)
    ...
    with pytest.raises(LockAcquisitionError, match="is currently held as shared"):
        minio_backend.acquire_lock(lock_key, exclusive_lock_id, timeout, shared=False)

    # Release shared lock
    minio_backend.release_lock(lock_key, shared_lock_id)

    # Now exclusive lock should succeed
    exclusive_result = minio_backend.acquire_lock(lock_key, exclusive_lock_id, timeout, shared=False)
    assert exclusive_result is True, "Should acquire exclusive lock after shared lock released"

    # Clean up
    minio_backend.release_lock(lock_key, exclusive_lock_id)


def test_lock_acquisition_stress_test(minio_backend, minio_client, minio_test_bucket):
    ...
    def stress_thread(thread_id: int):
        ...
        for iteration in range(num_iterations):
            ...
            try:
                # Try to acquire lock
                acquired = minio_backend.acquire_lock(lock_key, lock_id, timeout=5, shared=False)
                if acquired:
                    # Hold lock briefly
                    time.sleep(0.01)
                    # Release lock
    ...
    is_locked, _ = minio_backend.check_lock(lock_key)
...
def test_lock_retry_on_etag_mismatch(minio_backend, minio_client, minio_test_bucket):
    """Test that lock acquisition retries correctly when ETag mismatches occur.

    This test simulates a scenario where multiple processes try to acquire
    an expired lock simultaneously, causing ETag mismatches.
    """
    ...
    # Verify lock is held
    is_locked, current_lock_id = minio_backend.check_lock(lock_key)
