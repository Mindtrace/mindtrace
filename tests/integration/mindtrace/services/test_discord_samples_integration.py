...
class TestDiscordSamplesIntegration:
    ...
    def testing_token(self):
        ...
        if testing_token is None:
            pytest.skip(
                "MINDTRACE_TESTING_API_KEYS__DISCORD is not set. Please set it in order to run the Discord "
    ...
    def test_custom_discord_bot_service_commands(self, custom_discord_bot_service):
        ...
        if data["commands"]:
            ...
            for expected_cmd in expected_commands:
                assert any(expected_cmd in cmd_name for cmd_name in command_names), (
                    f"Expected command '{expected_cmd}' not found in {command_names}"
                )
        else:
            ...
            if "not connected" in response_text or "not found" in response_text or "error" in response_text:
                ...
                for expected in test_case["expected_terms"]:
                    assert expected.lower() in response_text, (
                        f"Expected '{expected}' not found in response: {data['response']}"
                    )
    ...
    def test_custom_discord_bot_service_unicode_handling(self, custom_discord_bot_service):
        ...
        unicode_commands = ["/roll 🎲", "/info 📊", "/help ❓", "/cleanup 🧹"]
    ...
    def test_custom_discord_bot_client_command_registration(self, custom_discord_bot_client):
        ...
        assert len(commands) == len(expected_commands), (
            f"Expected {len(expected_commands)} commands, got {len(commands)}"
        )
