...
def test_run_script_worker_both_environments(cluster_cm, node):
    ...
    wait_for_worker_launch(cluster_cm, str(node.url), launch1.launch_id, timeout=60.0)
    wait_for_worker_launch(cluster_cm, str(node.url), launch2.launch_id, timeout=60.0)
