...
``config.ini``). The test Docker stack uses ``mindtrace.core.testing.local_services``
(``LOCAL_MINIO_ENDPOINT``); ``scripts/docker_up.sh`` exports that when running
``ds test`` / ``run_tests.sh``.
"""
