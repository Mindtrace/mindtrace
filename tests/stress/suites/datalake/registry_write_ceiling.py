"""Datalake payload write-ceiling stress suite."""
...
def run(config: StressSuiteConfig, reporter: StressReporter) -> StressResult:
    """Measure Datalake payload writes without asset metadata insertion."""
...
def build_mount(
    ...
    if backend == "local":
        registry_path = Path(mkdtemp(prefix="mindtrace-datalake-payload-stress-"))
    ...
    raise ValueError(f"Unsupported datalake payload stress backend {backend!r}; expected local, minio, or gcs")
