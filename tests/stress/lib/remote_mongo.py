"""Remote MongoDB (Atlas) credentials for stress suites.

Uses Mindtrace-standard ``MINDTRACE_DATALAKE`` keys:
``REMOTE_MONGO_DB_URI`` (secret) and ``REMOTE_MONGO_DB_NAME``.
"""
...
def remote_mongo_from_core_config() -> tuple[str | None, str | None]:
    """Return ``(REMOTE_MONGO_DB_URI, REMOTE_MONGO_DB_NAME)`` from :class:`~mindtrace.core.config.CoreConfig` if set."""
...
def resolve_stress_atlas_mongo(resources: Mapping[str, Any], default_db_name: str) -> tuple[str, str]:
    """Pick Atlas URI and database for ``mongo_backend: atlas``.

    Preference order follows standard MT names, then runner legacy aliases, then
    optional run-scoped ``mongo_db_name``, then ``default_db_name``, then CoreConfig.
    """
    ...
    if uri is None:
        raise ValueError(
            "Stress Atlas Mongo requires REMOTE_MONGO_DB_URI / mongo_atlas_uri in resources, or "
