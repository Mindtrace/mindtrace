...
class TestEchoServiceThroughput:
    ...
    def test_concurrent_throughput_stress(self, echo_service_manager):
        ...
        results = {"successful": 0, "failed": 0, "response_times": [], "errors": []}
        ...
        def worker_function(worker_id):
            ...
            worker_results = {"successful": 0, "failed": 0, "response_times": [], "errors": []}
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            with tqdm(total=total_requests, desc="Concurrent requests", file=sys.stderr) as pbar:
                ...
                pbar.set_postfix({"completed": final_completed, "active_workers": 0})
                pbar.refresh()
        ...
        p95_response_time = (
            sorted(results["response_times"])[int(0.95 * len(results["response_times"]))]
            if results["response_times"]
            else 0
        )
        p99_response_time = (
            sorted(results["response_times"])[int(0.99 * len(results["response_times"]))]
            if results["response_times"]
            else 0
        )
        ...
        # Assertions
        assert results["successful"] > total_requests * 0.90, (
            f"Success rate too low: {results['successful']}/{total_requests}"
        )
    ...
    def test_sustained_load_stress(self, echo_service_manager):
        ...
        for bucket in sorted(time_buckets.keys()):
            if time_buckets[bucket]:
                ...
                print(
                    f"     {bucket:2d}-{bucket + 10:2d}s: avg={bucket_avg:.1f}ms, p95={bucket_p95:.1f}ms, count={len(time_buckets[bucket])}"
                )

        # Assertions
        assert total_successful > (test_duration * target_rps * 0.8), (
            f"Did not meet target RPS: {overall_rps:.1f} < {target_rps * 0.8}"
        )
        ...
        if len(time_buckets) >= 2:
            ...
            if time_buckets[first_bucket] and time_buckets[last_bucket]:
                ...
                print(
                    f"   - Performance change: {degradation:+.1f}% (first: {first_avg * 1000:.1f}ms → last: {last_avg * 1000:.1f}ms)"
                )
    ...
    def test_multi_worker_concurrent_throughput(self):
        ...
        for num_server_workers in worker_configs:
            ...
            try:
                with EchoService.launch(url="http://localhost:8091", timeout=15, num_workers=num_server_workers) as cm:
                    # Results tracking
                    test_results = {"successful": 0, "failed": 0, "response_times": [], "errors": []}
                    results_lock = threading.Lock()
                    completed_counter = {"count": 0}
                    counter_lock = threading.Lock()
                    ...
                    with ThreadPoolExecutor(max_workers=client_workers) as executor:
                        ...
                        with tqdm(total=total_requests, desc=f"{num_server_workers} workers", file=sys.stderr) as pbar:
                            ...
                            while any(not f.done() for f in futures):
                                ...
                                pbar.set_postfix({"completed": current_completed, "workers": num_server_workers})
                                pbar.refresh()
                    ...
                    p95_response_time = (
                        sorted(test_results["response_times"])[int(0.95 * len(test_results["response_times"]))]
    ...
    def test_delayed_processing_multi_worker_throughput(self):
        ...
        for num_server_workers in worker_configs:
            ...
            # Launch service with specific number of workers
            try:
                with EchoService.launch(url="http://localhost:8093", timeout=15, num_workers=num_server_workers) as cm:
                    # Results tracking
                    test_results = {"successful": 0, "failed": 0, "response_times": [], "errors": []}
                    results_lock = threading.Lock()
                    completed_counter = {"count": 0}
        ...
        for workers, data in results.items():
            if data:
                ...
                print(
                    f"{workers:<8} {throughput:<12.1f} {scaling:<10} {efficiency:<12} {avg_rt:<10.1f} {success:<8.1f}%"
                )
        ...
        summary_results = {
            ...
            "summary": {
                "configurations_tested": len([r for r in results.values() if r is not None]),
                "failed_configurations": len([r for r in results.values() if r is None]),
                "delay_working": min(r["avg_response_time"] for r in results.values() if r is not None)
        ...
        assert min_avg_response >= processing_delay * 0.9, (
            f"Processing delay not working: {min_avg_response * 1000:.1f}ms < {processing_delay * 1000 * 0.9:.1f}ms"
        )
    ...
    def test_single_request_baseline(self, echo_service_manager):
        ...
        for delay in test_delays:
            ...
            if response_times:
                ...
                print(
                    f"{delay * 1000:<8.0f} {expected_time:<10.0f} {avg_response_time:<10.1f} {overhead:<10.1f} {efficiency:<10.1f}%"
                )
            else:
        ...
        summary_results = {
            ...
            "test_config": {"test_delays_ms": [d * 1000 for d in test_delays], "requests_per_delay": 5},
