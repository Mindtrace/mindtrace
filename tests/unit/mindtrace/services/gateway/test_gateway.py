...
class TestGateway:
    ...
    def test_init_adds_register_app_endpoint(self, mock_add_endpoint):
        ...
        register_app_calls = [
            call for call in mock_add_endpoint.call_args_list 
            if len(call[0]) > 0 and 'register_app' in str(call[0][0])
        ]
            ...
            with patch.object(Gateway, 'status_at_host', return_value=ServerStatus.AVAILABLE):
                ...
                with patch('mindtrace.services.gateway.gateway.ProxyConnectionManager') as mock_proxy_class:
                    ...
                    result = enhanced_cm.register_app(
                        ...
                        connection_manager=mock_original_cm
                    )
...
class TestProxyConnectionManagerIntegration:
    ...
    def test_proxy_method_call_no_args(self, proxy_cm):
        ...
        with patch('requests.get') as mock_get:
            ...
            # Mock the original method to have no required parameters
            mock_method = Mock()
            mock_method.__name__ = 'status'
            
            with patch('inspect.signature') as mock_signature:
                mock_param = Mock()
                mock_param.default = Mock()  # Has default, so not required
                mock_signature.return_value.parameters.values.return_value = [mock_param]
                
                # Test calling a no-arg method
                result = proxy_cm.status()
                
                # Verify GET was used instead of POST
    ...
    def test_proxy_missing_method(self, proxy_cm):
        """Test calling non-existent method through proxy."""
        with pytest.raises(AttributeError) as exc_info:
            proxy_cm.nonexistent_method()
