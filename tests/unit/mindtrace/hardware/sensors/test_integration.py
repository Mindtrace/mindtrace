...
from mindtrace.hardware.sensors.core.factory import create_backend, create_simulator_backend
...
class TestSensorSystemIntegration:
    ...
    async def test_mixed_backend_types_integration(self, sample_mqtt_config):
        ...
        with patch('mindtrace.hardware.sensors.core.manager.create_backend') as mock_create:
            ...
            mqtt_sensor = manager.register_sensor("mqtt_sensor", "mqtt", sample_mqtt_config, "mqtt/topic")
            http_sensor = manager.register_sensor("http_sensor", "http", {"base_url": "http://api.com"}, "/data")
            serial_sensor = manager.register_sensor("serial_sensor", "serial", {"port": "/dev/ttyUSB0"}, "/dev/ttyUSB0")
