...
from mindtrace.hardware.services.sensors.connection_manager import SensorConnectionManager
...
class TestSensorConnectionManager:
    ...
    async def test_list_sensors_without_status(self):
        ...
        mock_response = {"success": True, "sensors": [], "count": 0, "message": "Retrieved 0 sensors"}
    ...
    async def test_connect_mqtt_sensor_convenience_method(self):
        ...
        with patch.object(manager, "call_endpoint", new_callable=AsyncMock, return_value=mock_response):
            ...
            request = args[0][1]
            assert request.backend_type == "mqtt"
            assert request.config == {"broker_url": "mqtt://localhost:1883", "identifier": "test_client"}
    ...
    async def test_connect_http_sensor_no_headers(self):
        ...
        with patch.object(manager, "call_endpoint", new_callable=AsyncMock, return_value=mock_response):
            await manager.connect_http_sensor(
                sensor_id="http_sensor", base_url="http://api.sensors.com", address="/sensors/temperature"
            )
    ...
    async def test_connect_serial_sensor_convenience_method(self):
        ...
        with patch.object(manager, "call_endpoint", new_callable=AsyncMock, return_value=mock_response):
            result = await manager.connect_serial_sensor(
                sensor_id="serial_sensor", port="/dev/ttyUSB0", baudrate=115200, timeout=2.0
            )
            ...
            assert request.config == {"port": "/dev/ttyUSB0", "baudrate": 115200, "timeout": 2.0}
    ...
    async def test_connect_serial_sensor_default_params(self):
        ...
        with patch.object(manager, "call_endpoint", new_callable=AsyncMock, return_value=mock_response):
            await manager.connect_serial_sensor(sensor_id="serial_sensor", port="/dev/ttyUSB0")
    ...
    async def test_multiple_concurrent_operations(self):
        ...
        mock_responses = [
            {"success": True, "sensor_id": "sensor1", "status": "connected", "message": "Connected"},
            {
                "success": True,
                "sensor_id": "sensor2",
                "data": {"temp": 25.0},
            ...
            {"success": True, "sensors": [], "count": 0, "message": "No sensors"},
        ]
    ...
    async def test_convenience_methods_parameter_handling(self):
        ...
        with patch.object(manager, "call_endpoint", new_callable=AsyncMock, return_value=mock_response):
            ...
            await manager.connect_mqtt_sensor(
                sensor_id="mqtt_test", broker_url="mqtt://broker:1883", identifier="client_123", address="sensors/data"
            )
            ...
            await manager.connect_serial_sensor(sensor_id="serial_test", port="COM1", baudrate=115200, timeout=5.0)
