...
from mindtrace.hardware.core.exceptions import CameraInitializationError, CameraConfigurationError
from tests.unit.mindtrace.hardware.mocks.hardware_test_utils import create_test_image, assert_image_valid
...
def test_sync_camera_capture_and_config():
    ...
    try:
        ...
        assert set(required_keys).issubset(info.keys()), f"Missing required info keys: {required_keys - set(info.keys())}"
        ...
        assert isinstance(backend_name, str) and len(backend_name) > 0, f"Backend should resolve to non-empty string, got {backend_name}"
        ...
        if isinstance(hdr_result, tuple):
            ...
            if images:
                assert len(images) == len(exposure_levels), f"Should have equal images and exposure levels: {len(images)} vs {len(exposure_levels)}"
                for img in images:
                    assert_image_valid(img, expected_dtype=np.uint8)
...
def test_camera_configuration_persistence():
    ...
    try:
        ...
        if trigger_ok:
            ...
            assert actual_mode == test_trigger_mode, f"Trigger mode not persisted: expected {test_trigger_mode}, got {actual_mode}"
...
def test_camera_roi_operations():
    ...
    try:
        ...
        if roi_set:
            ...
            assert updated_roi["height"] == new_height, f"ROI height: expected {new_height}, got {updated_roi['height']}"
