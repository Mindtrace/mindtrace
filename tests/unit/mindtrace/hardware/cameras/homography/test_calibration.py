...
class TestHomographyCalibrator:
    ...
    def test_estimate_intrinsics_from_fov(self):
        ...
        K = self.calibrator.estimate_intrinsics_from_fov(
            image_size=image_size, fov_horizontal_deg=fov_horizontal, fov_vertical_deg=fov_vertical
        )
    ...
    def test_estimate_intrinsics_custom_principal_point(self):
        ...
        K = self.calibrator.estimate_intrinsics_from_fov(
            image_size=image_size, fov_horizontal_deg=60.0, fov_vertical_deg=45.0, principal_point=custom_pp
        )
    ...
    def test_calibrate_from_correspondences_basic(self):
        ...
        image_points = np.array(
            [[100.0, 200.0], [300.0, 200.0], [300.0, 400.0], [100.0, 400.0], [200.0, 300.0]], dtype=np.float64
        )
    ...
    def test_calibrate_from_correspondences_with_camera_matrix(self):
        ...
        world_points = np.array([[0.0, 0.0], [50.0, 0.0], [50.0, 50.0], [0.0, 50.0]], dtype=np.float64)

        image_points = np.array([[100.0, 100.0], [200.0, 105.0], [195.0, 200.0], [105.0, 195.0]], dtype=np.float64)
    ...
    def _test_checkerboard_with_mock(self):
        ...
        with patch("cv2.findChessboardCorners") as mock_find, patch("cv2.findHomography") as mock_homography:
            ...
            # Mock homography computation
            H = np.array([[2.0, 0.0, 100.0], [0.0, 2.0, 50.0], [0.0, 0.0, 1.0]], dtype=np.float64)
            ...
            calib_data = self.calibrator.calibrate_checkerboard(
                image=image, board_size=(5, 3), square_size=25.0, world_unit="cm", refine_corners=False
            )
    ...
    def test_various_world_units(self):
        ...
        for unit in ["mm", "cm", "m", "in", "ft"]:
            calib_data = self.calibrator.calibrate_from_correspondences(
                world_points=world_points, image_points=image_points, world_unit=unit
            )
    ...
    def _test_pil_image_with_mock(self):
        ...
        with patch("cv2.findChessboardCorners") as mock_find, patch("cv2.findHomography") as mock_homography:
            ...
            calib_data = self.calibrator.calibrate_checkerboard(
                image=pil_image, board_size=(5, 3), square_size=25.0, world_unit="mm", refine_corners=False
            )
