...
class TestMockBaslerImageGeneration:
    ...
    async def test_auto_pattern_rotation(self):
        ...
        assert len(unique_stats) > 1, f"Expected pattern rotation to create different images, got {len(unique_stats)} unique patterns from {len(image_stats)} captures"
        await camera.close()
    ...
    async def test_checkerboard_size_parameter(self):
        ...
        camera = MockBaslerCameraBackend(
            "test_cam",
            synthetic_pattern="checkerboard",
            synthetic_checker_size=100
        )
        await camera.initialize()
    ...
    async def test_text_overlay_toggle(self):
        ...
        # With text overlay
        camera_with_text = MockBaslerCameraBackend(
            "test_cam_text",
            synthetic_overlay_text=True
        )
        await camera_with_text.initialize()
        
        # Without text overlay  
        camera_no_text = MockBaslerCameraBackend(
            "test_cam_no_text", 
        ...
        # Both should work
        success1, _ = await camera_with_text.capture()
        success2, _ = await camera_no_text.capture()
...
class TestMockBaslerConfigurationValidation:
    ...
    async def test_trigger_mode_validation(self):
        ...
        success = await camera.set_triggermode("continuous")
        assert success is True
        
        success = await camera.set_triggermode("trigger")
        assert success is True
...
class TestMockBaslerConfigurationFiles:
    ...
    async def test_export_config_directory_creation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            success = await camera.export_config(config_path)
            assert success is True
    ...
    async def test_partial_configuration_import(self):
        ...
        try:
            success = await camera.import_config(partial_path)
            assert success is True
...
class TestMockBaslerWhiteBalance:
    ...
    async def test_white_balance_get_set(self):
        ...
        for mode in wb_modes:
            success = await camera.set_auto_wb_once(mode)
            assert success is True
...
class TestMockBaslerEdgeCasesAndErrorHandling:
    ...
    async def test_concurrent_operations(self):
        ...
        capture_results, _ = await asyncio.gather(
            asyncio.gather(*[capture_task() for _ in range(3)]),
            config_task()
        )
        
        # All captures should succeed
        for success, image in capture_results:
            assert success is True
            assert isinstance(image, np.ndarray)
