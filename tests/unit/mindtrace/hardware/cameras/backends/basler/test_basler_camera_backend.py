...
class MockPylonCamera:
    ...
    def __init__(self, device_info=None):
        ...
        self._exposure_time_param = MockParameter(
        ...
        self._pixel_format_param = MockEnumParameter(
            self.pixel_format, ["Mono8", "BGR8", "RGB8"], camera_attr="pixel_format", camera_obj=self
        )
        self._trigger_mode_param = MockEnumParameter(
            self.trigger_mode, ["Off", "On"], camera_attr="trigger_mode", camera_obj=self
        )
        self._trigger_source_param = MockEnumParameter(
            self.trigger_source, ["Software", "Line1", "Line2"], camera_attr="trigger_source", camera_obj=self
        )
        self._max_num_buffer_param = MockParameter(
            25, min_val=1, max_val=100, camera_attr="max_num_buffer", camera_obj=self
        )
        self._trigger_selector_param = MockEnumParameter(
            "FrameStart", ["FrameStart", "LineStart"], camera_attr="trigger_selector", camera_obj=self
        )
        self._trigger_software_param = MockParameter(None, camera_attr="trigger_software", camera_obj=self)
        self._balance_white_auto_param = MockEnumParameter(
            "Off", ["Off", "Once", "Continuous"], camera_attr="balance_white_auto", camera_obj=self
        )
...
def mock_pypylon(monkeypatch):
    ...
    # Mock TlFactory
    tl_factory = Mock()
    devices = [MockPylonDevice(serial=f"1234567{i}", model="acA1920-40uc") for i in range(3)]
...
class TestBaslerCameraBackendConfiguration:
    ...
    async def test_get_gamma_without_node_returns_linear_default(self, basler_camera):
        ...
        assert await basler_camera.get_gamma_range() == [0.25, 2.0]
...
class TestBaslerCameraBackendMissingCoverageLines:
    ...
    async def test_run_blocking_method_hardware_operation_error_branch(self, basler_camera):
        ...
        with pytest.raises(HardwareOperationError, match="operation failed"):
...
class TestBaslerCameraBackendUncoveredErrorPaths:
    ...
    async def test_run_blocking_timeout_error_conversion(self, basler_camera, monkeypatch):
        ...
        with pytest.raises(CameraTimeoutError, match="operation timed out after .* for camera"):
    ...
    async def test_run_blocking_generic_exception_conversion(self, basler_camera, monkeypatch):
        ...
        with pytest.raises(HardwareOperationError, match="operation failed for camera"):
            await basler_camera._run_blocking(lambda: None, timeout=1.0)
