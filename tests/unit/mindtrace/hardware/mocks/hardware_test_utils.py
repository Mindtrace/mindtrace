...
def simulate_hardware_error(device: MockHardwareDevice, error_type: str, 
                           auto_clear: bool = False, clear_after: float = 1.0):
    """
...
def create_test_image(width: int = 1920, height: int = 1080, 
                     channels: int = 3, dtype: type = np.uint8,
...
def assert_image_valid(image: np.ndarray, expected_shape: Optional[Tuple] = None,
    ...
    if expected_range is not None:
        ...
        assert actual_min >= min_val, \
            f"Image min value {actual_min} below expected {min_val}"
        assert actual_max <= max_val, \
            f"Image max value {actual_max} above expected {max_val}"
...
def create_mock_service_response(success: bool = True, data: Any = None, 
...
def verify_error_propagation(exception_class: type, error_message: str,
    ...
    try:
        ...
        assert error_message in str(e), \
            f"Expected error message containing '{error_message}', got '{str(e)}'"
    except Exception as e:
        raise AssertionError(
            f"Expected {exception_class.__name__}, got {type(e).__name__}: {str(e)}"
        )
