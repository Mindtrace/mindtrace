...
class Datum(MindtraceDocument):
    ...
    data: Any = Field(
        default=None,
        description="The data content of this datum. Can be None if stored in a registry."
    )
    registry_uri: str | None = Field(
        default=None,
        description="URI of the registry backend where this datum is stored."
    )
    registry_key: str | None = Field(
        default=None,
        description="Unique key within the registry for retrieving this datum's data."
    )
    derived_from: Annotated[PydanticObjectId | None, Indexed(unique=False)] = Field(
        default=None,
        description="ID of the parent datum this datum was derived from."
    )
