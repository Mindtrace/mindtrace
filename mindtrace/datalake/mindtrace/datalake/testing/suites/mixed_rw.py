...
class DatalakeMixedRwResources(MinioBenchResources):
...
class DatalakeMixedRwSuite(BenchTestSuite):
    ...
    def execute_bench(self, config: BenchSuiteConfig, reporter: BenchReporter) -> BenchResult:
        ...
        try:
            ...
            lake.initialize()
            refs = [
                lake.put_object(name=f"{prefix}/seed/{index:08d}", obj=payload, mount="stress")
            ...
            def operation() -> None:
                ...
                try:
                    if is_read:
                        ...
                        ref = lake.put_object(
                            name=f"{prefix}/write/{write_index:08d}-{uuid4().hex}", obj=payload, mount="stress"
                        )
                        refs.append(ref)
                        write_index += 1
                        write_ops += 1
