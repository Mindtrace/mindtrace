"""Datalake collection item creation throughput benchmark."""
...
class DatalakeCollectionItemInput(BaseModel):
...
class DatalakeCollectionItemSuite(BenchTestSuite):
    suite_id = "datalake.stress.collection_item"
    title = "Datalake stress — collection item creation"
    description = "Measures collection membership creation over pre-seeded assets."
    ...
    task_schema = TaskSchema(name=suite_id, input_schema=DatalakeCollectionItemInput, output_schema=BenchResultSchema)
    resource_schema = DatalakeCollectionItemResources
    ...
    def execute_bench(self, config: BenchSuiteConfig, reporter: BenchReporter) -> BenchResult:
        ...
        try:
            ...
            lake.initialize()
            collection = lake.create_collection(name=f"bench-{config.run_id}-{uuid4().hex}", metadata={"bench_run_id": config.run_id})
            assets = [
            ...
            def operation() -> None:
                ...
                try:
                    item = lake.create_collection_item(
                        collection_id=collection.collection_id,
                        asset_id=asset.asset_id,
                        split="train",
                        metadata={"bench_run_id": config.run_id, "sequence": index},
                    )
                    lake.resolve_collection_item(item.collection_item_id)
