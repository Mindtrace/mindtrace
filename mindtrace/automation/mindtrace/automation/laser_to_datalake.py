...
def download_and_process_image(args):
    d, images_save_path, masks_save_path, class2idx, class_names = args
    ...
    # Check for 'unsure' labels
    has_unsure_label = False
    for an in d.get('annotations', []):
        for result in an.get('result', []):
            ...
            class_name = labels[0]
            if 'unsure' in class_name.lower() or 'ignore' in class_name.lower() or 'unknown' in class_name.lower():
                has_unsure_label = True
                break
        if has_unsure_label:
            break

    if has_unsure_label:
        unsure_dir = os.path.join(os.path.dirname(images_save_path), 'unsure')
        os.makedirs(unsure_dir, exist_ok=True)
        if os.path.exists(image_local_path):
            shutil.move(image_local_path, os.path.join(unsure_dir, fname))
            print(f"Moved image with 'unsure' label to {unsure_dir}: {fname}")
        else:
            print(f"Could not find image to move: {image_local_path}")
        return None
...
def download_data(
    ...
    tasks = [(d, images_save_path, masks_save_path, class2idx, class_names) for d in data]
...
def train_test_split(masks_save_path, images_save_path, desired_ratio=0.2):
    ...
    for mask in train_set:
        try:
            ...
            image_name = mask.replace('_mask.png', '.png')
...
def upload_to_huggingface(download_dir, huggingface_config, class_names, clean_up=True, remove_holes=True, hole_id=1):
    ...
    if existing_dataset is not None:
        ...
        for file in os.listdir(os.path.join(datalake_train_path, 'images')):
            ...
            mask_path = os.path.join(datalake_train_path, 'masks', file.replace('.png', '_mask.png'))
            if remove_holes:
            ...
            shutil.move(mask_path, os.path.join(download_dir, 'masks', 'train', file.replace('.png', '_mask.png')))

        for file in os.listdir(os.path.join(datalake_test_path, 'images')):
            image_path = os.path.join(datalake_test_path, 'images', file)
            mask_path = os.path.join(datalake_test_path, 'masks', file.replace('.png', '_mask.png'))
            if remove_holes:
                mask = cv2.imread(mask_path)
                mask[mask == hole_id] = 0
                cv2.imwrite(mask_path, mask)
            ...
            shutil.move(mask_path, os.path.join(download_dir, 'masks', 'test', file.replace('.png', '_mask.png')))
    ...
    for k, v in name2c.items():
        if max(v) > 10:
            train_type[k.replace('_mask.png', '.png')] = 'OB'
        else: 
            train_type[k.replace('_mask.png', '.png')] = 'IB'
    ...
    for k, v in name2c.items():
        if max(v) > 10:
            test_type[k.replace('_mask.png', '.png')] = 'OB'
        else: 
            test_type[k.replace('_mask.png', '.png')] = 'IB'
    ...
    for i in os.listdir(source_images_train):
        ...
        train_data_files[i.replace('.png', '_mask.png')] = os.path.join(target_path, 'splits', 'train', 'masks', i.replace('.png', '_mask.png'))
        shutil.copy(os.path.join(source_masks_train, i.replace('.png', '_mask.png')), os.path.join(target_path, 'splits', 'train', 'masks', i.replace('.png', '_mask.png')))
    ...
    for i in os.listdir(source_images_test):
        ...
        val_data_files[i.replace('.png', '_mask.png')] = os.path.join(target_path, 'splits', 'test', 'masks', i.replace('.png', '_mask.png'))
        shutil.copy(os.path.join(source_masks_test, i.replace('.png', '_mask.png')), os.path.join(target_path, 'splits', 'test', 'masks', i.replace('.png', '_mask.png')))
...
if __name__ == "__main__":
    ...
    # unique_id = "9d8178f4-c0e9-4e4b-a98b-89c5c40541ed"
