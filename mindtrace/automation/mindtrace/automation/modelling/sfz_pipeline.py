...
class SFZPipeline:
    ...
    def run_inference_on_path(self, 
                            ...
                            image_list: Optional[List[str]] = None) -> Dict[str, Any]:
        """Run inference on a single image, a list of images, or all images in a folder.
        ...
        Args:
            ...
            supported_formats: Image formats to process if scanning a folder.
            image_list: A specific list of image paths to process. If provided, `input_path` is ignored for image discovery.
            
        Returns:
            Dictionary with results summary.
        """
        if not os.path.exists(input_path) and not image_list:
            raise ValueError(f"Input path does not exist and no image_list provided: {input_path}")
        
        # If a specific list of images is provided, use it.
        if image_list:
            # The list might contain URI format `file://`, so we need to clean it up.
            cleaned_image_list = [p.replace('file://', '') for p in image_list]
            return self.run_inference_on_folder(
                input_folder=None,  # Not needed since we have a list
                output_folder=output_folder,
                export_types=export_types,
                threshold=threshold,
                save_visualizations=save_visualizations,
                supported_formats=supported_formats,
                image_files_list=cleaned_image_list
            )
        
        # Original logic for handling a single file or a directory
        if os.path.isfile(input_path):
            ...
            raise ValueError(f"Input path is neither a file, directory, nor is an image_list provided: {input_path}")
    ...
    def run_inference_on_folder(
        ...
        supported_formats: Tuple[str, ...] = ('.jpg', '.jpeg', '.png', '.bmp', '.tiff', '.webp'),
        num_workers: int = 4,
        image_files_list: Optional[List[str]] = None) -> Dict[str, Any]:
        """Run inference concurrently on a list of images or all images in a folder.
        
        Args:
            input_folder: Path to folder containing images (used if image_files_list is None).
            output_folder: Path to save results and visualizations.
            export_types: Dictionary mapping task names to export types.
            threshold: Confidence threshold for detections.
            save_visualizations: Whether to save visualization images.
            ...
            image_files_list: A specific list of image paths to process.
        ...
        # Determine the list of image files to process
        if image_files_list:
            image_files = image_files_list
            print(f"Processing {len(image_files)} images from the provided list using {num_workers} workers.")
            ...
            print("No valid input folder or image list provided.")
            return {'error': 'No input provided'}

        if not image_files:
            print(f"No supported image files found to process.")
