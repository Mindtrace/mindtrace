        ...
        if 'bboxes' in image_data and image_data['bboxes']:
            ...
            for bbox_data in image_data['bboxes']:
                try:
                    ...
                    if bbox_data["task_type"] == "spatter_segmentation":
                    ...
                    if bbox_data["task_type"] == "weld_classification":
                            x = bbox_data['x']
                            y = bbox_data['y']
                            width = bbox_data['width']
                            height = bbox_data['height']
                            ...
                            # UNCOMMENT IF WELD CLASSIFICATION IS ALSO NEEDED
                            # image_annotations.append(rect_annotation)

                    if bbox_data["task_type"] == "defect_classification":
                            x = bbox_data['x']
                            y = bbox_data['y']
                            width = bbox_data['width']
                            height = bbox_data['height']
...
def parse_yolo_box_file(pipeline: str, task_name:str, box_file_path: str, img_width: int, img_height: int, id2label: dict) -> List[Dict[str, Any]]:
    """
        ...
        with open(box_file_path, 'r') as f:
            for line in f:
                ...
                if len(parts) >= 5:
                    ...
                    if pipeline == "mig":
                        if task_name == "weld_classification":
                        ...
                        if task_name == "defect_classification":
                            class_id = int(float(parts[0]))
                            class_name = str(parts[1])
                            center_x = float(parts[2])
                            center_y = float(parts[3])
...
def gather_detections_from_folders(
    ...
    images_folder = os.path.join(output_folder,job_id, "images")
    boxes_folder = os.path.join(output_folder,job_id, "boxes")
    raw_masks_folder = os.path.join(output_folder,job_id, "raw_masks")
    ...
    for image_file in image_files:
        ...
        if box_task_names:
            for task_name in box_task_names:
                ...
                bboxes.extend(parse_yolo_box_file(pipeline,task_name,box_file_path, img_width, img_height, id2label))
