...
def resolve_provider_config(provider_key: str, providers: Dict[str, Any]) -> dict:
    if provider_key not in providers:
        raise KeyError(
            f"Provider '{provider_key}' not found in providers. "
...
def get_ollama_model(model_id: str, **kwargs):
    ...
    try:
        ...
        raise ImportError(
            f"Ollama model requires the 'ollama' package to be installed. {e} "
...
def get_openai_model(model_id: str, api_key: str = None, **kwargs):
    ...
    try:
        ...
        raise ImportError(
            f"OpenAI model requires the 'openai' package to be installed. {e} "
...
def get_model_from_provider(provider_config: dict, model_name: str, **kwargs) -> Any:
    ...
    match provider_type:
        case "ollama":
            model_kwargs = {
                "host": provider_config.get("base_url", "http://localhost:11434"),
                **kwargs
            }
