...
class MonitorAgent(BaseAgent):
    ...
    def send_loki_request(self, loki_url: str, logql: str, start_time: str, end_time: str, log_limit: int) -> Dict[str, Any]:
    ...
    async def query(self, query: str, service: str) -> dict:
        ...
        if not res["success"]:
            if res["error"] == "No logs found":
                return {"AI_message": f"Logs for provided service: {service} not found for last 7 days in loki {self.config.get("LOKI_URL")}. Please Confirm the service name and if the service is running"}
            else:
