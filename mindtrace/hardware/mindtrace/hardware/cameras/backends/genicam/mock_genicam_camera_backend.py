...
class MockGenICamCameraBackend(CameraBackend):
    ...
    def __init__(
        ...
        **backend_kwargs,
        ...
        self.fail_init = backend_kwargs.get("simulate_fail_init", os.getenv("MOCK_GENICAM_FAIL_INIT", "false").lower() == "true")
        self.fail_capture = backend_kwargs.get("simulate_fail_capture", os.getenv("MOCK_GENICAM_FAIL_CAPTURE", "false").lower() == "true")
        self.simulate_timeout = backend_kwargs.get("simulate_timeout", os.getenv("MOCK_GENICAM_TIMEOUT", "false").lower() == "true")
    ...
    def _setup_vendor_quirks(self) -> Dict[str, bool]:
        ...
        if 'KEYENCE' in self.vendor.upper():
            ...
            self.logger.debug(f"Mock camera using Keyence quirks (integer exposure)")
            ...
            self.logger.debug(f"Mock camera using Basler quirks (float exposure)")
    ...
    async def reset_ROI(self):
        ...
        self.logger.debug(f"Mock camera ROI reset to maximum")
    ...
    async def import_config(self, config_path: str):
        ...
        try:
            ...
            if "roi" in config_data:
                roi = config_data["roi"]
                await self.set_ROI(roi.get("x", 0), roi.get("y", 0), roi.get("width", self.synthetic_width), roi.get("height", self.synthetic_height))
