...
class PylonSDKInstaller(Mindtrace):
    ...
    WINDOWS_SDK_URL = "https://github.com/Mindtrace/basler-sdk/releases/download/basler_sdk_windows/Basler.pylon.8.1.0.zip"
    ...
    def is_sdk_installed(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.debug(f"pypylon not available or not functional: {e}")
                pypylon_working = False
            ...
            if sdk_lib_found and pypylon_working:
                ...
                self.logger.info("SDK library found but pypylon not available")
                return False
            elif pypylon_working:
                self.logger.info("pypylon available but SDK library not in standard locations")
                return True  # pypylon might include bundled libraries
            else:
...
For more information, visit: https://www.baslerweb.com/en/downloads/software-downloads/
    ...
    parser.add_argument(
        "--uninstall", 
        action="store_true", 
        help="Uninstall the Pylon SDK instead of installing"
    )
    parser.add_argument(
        "--version",
        default="v1.0-stable",
