...
class MockAllenBradleyPLC(BasePLC):
    ...
    def __init__(
        ...
        self.logger.info(
            f"Mock Allen Bradley PLC initialized: plc_type={self.plc_type}, "
            ...
            for tag_name in tag_list:
                if tag_name in self._tag_values:
                    ...
                    if isinstance(value, (int, float)) and any(
                        keyword in tag_name.lower() 
                        for keyword in ['temp', 'pressure', 'speed', 'level']
                    ):
    ...
    async def get_all_tags(self) -> List[str]:
        ...
        try:
            if self.driver_type == "LogixDriver":
                ...
                self._tags_cache = [
                    tag for tag in self._tag_values.keys() 
                    if not any(char in tag for char in [':', '.', '/'])
                ]
            elif self.driver_type == "SLCDriver":
                ...
                self._tags_cache = [
                    tag for tag in self._tag_values.keys() 
                    if any(tag.startswith(prefix) for prefix in ['Assembly:', 'Parameter:', 'Module:', 'Connection', '0x', 'Identity', 'DeviceInfo'])
                ]
