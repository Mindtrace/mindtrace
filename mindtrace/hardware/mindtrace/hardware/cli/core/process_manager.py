...
class ProcessManager:
    ...
    def start_camera_api(self, host: str = None, port: int = None, 
                        include_mocks: bool = False) -> subprocess.Popen:
        ...
        cmd = [
            sys.executable, '-m', 
            'mindtrace.hardware.api.cameras.launcher',
            '--host', host,
            '--port', str(port)
        ]
    ...
    def start_configurator(self, host: str = None, port: int = None,
                          backend_port: int = None) -> subprocess.Popen:
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cmdline', 'cwd']):
                try:
                    ...
                    if any(pattern in cmdline_str for pattern in [
                        'reflex', 'rx', 'camera_configurator', 'camera-configurator'
                    ]):
                        should_kill = True
                    ...
                    elif any(pattern in cmdline_str for pattern in [
                        f":{info.get('port', 3000)}", 
                        f":{info.get('backend_port', 8000)}"
                    ]):
                        should_kill = True
