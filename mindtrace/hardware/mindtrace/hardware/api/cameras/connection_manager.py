...
class CameraManagerConnectionManager(ConnectionManager):
    ...
    async def capture_image(self, camera: str, save_path: Optional[str] = None, upload_to_gcs: bool = False, output_format: str = "numpy") -> Dict[str, Any]:
        ...
        Returns:
            Capture result
        """
        request = CaptureImageRequest(camera=camera, save_path=save_path, upload_to_gcs=upload_to_gcs, output_format=output_format)
        response = await self.post("/cameras/capture", request.model_dump(), timeout=120.0)
        return response["data"]
    
    async def capture_images_batch(self, cameras: List[str], upload_to_gcs: bool = False, output_format: str = "numpy") -> Dict[str, Any]:
    ...
    async def capture_hdr_image(self, camera: str, save_path_pattern: Optional[str] = None, 
                               exposure_levels: int = 3, exposure_multiplier: float = 2.0,
                               return_images: bool = True, upload_to_gcs: bool = False, 
        ...
        request = CaptureHDRRequest(
            camera=camera, save_path_pattern=save_path_pattern,
    ...
    async def capture_hdr_images_batch(self, cameras: List[str], save_path_pattern: Optional[str] = None,
        ...
        request = CaptureHDRBatchRequest(
            cameras=cameras, save_path_pattern=save_path_pattern,
            exposure_levels=exposure_levels, exposure_multiplier=exposure_multiplier,
            return_images=return_images, upload_to_gcs=upload_to_gcs,
            output_format=output_format
        )
