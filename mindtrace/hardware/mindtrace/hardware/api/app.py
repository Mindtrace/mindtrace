...
async def get_camera_info(camera: str) -> StatusResponse:
    ...
    return StatusResponse(success=True, data=info, message=f"Camera information retrieved for '{camera}'")
...
async def capture_image(camera: str, payload: CaptureRequest = None) -> CaptureResponse:
    ...
    capture_result = await camera_proxy.capture(
        save_path=save_path, gcs_bucket=gcs_bucket, gcs_path=gcs_path, gcs_metadata=gcs_metadata
    )
    ...
    for camera_name, hdr_result in hdr_results.items():
        if payload.return_images and isinstance(hdr_result, dict) and hdr_result.get("images"):
            ...
            if not payload.return_images:
                message += " (image data excluded)"
            response_results[camera_name] = HDRCaptureResponse(
                success=hdr_result, message=message, successful_captures=successful_captures
            )
        else:
            response_results[camera_name] = HDRCaptureResponse(
...
async def get_white_balance_modes(camera: str) -> WhiteBalanceListResponse:
    ...
    return WhiteBalanceListResponse(success=True, data=modes, message="Available white balance modes")
...
async def set_roi(camera: str, payload: ROIRequest) -> BoolResponse:
    ...
    if not success:
        raise CameraConfigurationError("Failed to set ROI")
    return BoolResponse(
        success=True, message=f"ROI set to ({payload.x}, {payload.y}, {payload.width}, {payload.height})"
    )
...
async def reset_roi(camera: str) -> BoolResponse:
    ...
    if not success:
        raise CameraConfigurationError("Failed to reset ROI")
...
async def get_pixel_formats(camera: str) -> PixelFormatListResponse:
    ...
    return PixelFormatListResponse(success=True, data=formats, message="Available pixel formats")
...
async def get_image_enhancement(camera: str) -> BoolResponse:
    ...
    return BoolResponse(
        success=True, data=enabled, message=f"Image enhancement: {'enabled' if enabled else 'disabled'}"
    )
...
async def set_image_enhancement(camera: str, payload: ImageEnhancementRequest) -> BoolResponse:
    ...
    if not success:
        raise CameraConfigurationError("Failed to set image enhancement")
...
async def export_config(camera: str, payload: ConfigFileRequest) -> BoolResponse:
    ...
    if not success:
        raise CameraConfigurationError("Failed to export configuration")

    return BoolResponse(success=True, message=f"Configuration exported to '{payload.config_path}'")
...
async def import_config(camera: str, payload: ConfigFileRequest) -> BoolResponse:
    ...
    if not success:
        raise CameraConfigurationError("Failed to import configuration")

    return BoolResponse(success=True, message=f"Configuration imported from '{payload.config_path}'")
...
initialize_camera_task = TaskSchema(
    name="initialize_camera", input_schema=CameraInitializeRequest, output_schema=BoolResponse
)
initialize_cameras_batch_task = TaskSchema(
    name="initialize_cameras_batch", input_schema=BatchCameraInitializeRequest, output_schema=BatchOperationResponse
)
...
check_camera_connection_task = TaskSchema(
    name="check_camera_connection", input_schema=None, output_schema=StatusResponse
)
...
get_specific_backend_info_task = TaskSchema(
    name="get_specific_backend_info", input_schema=None, output_schema=DictResponse
)
...
capture_batch_task = TaskSchema(
    name="capture_batch", input_schema=BatchCaptureRequest, output_schema=BatchOperationResponse
)
capture_hdr_task = TaskSchema(name="capture_hdr", input_schema=HDRCaptureRequest, output_schema=HDRCaptureResponse)
capture_hdr_batch_task = TaskSchema(
    name="capture_hdr_batch", input_schema=BatchHDRCaptureRequest, output_schema=BatchHDRCaptureResponse
)
...
set_white_balance_task = TaskSchema(
    name="set_white_balance", input_schema=WhiteBalanceRequest, output_schema=BoolResponse
)
get_white_balance_modes_task = TaskSchema(
    name="get_white_balance_modes", input_schema=None, output_schema=WhiteBalanceListResponse
)
configure_batch_async_task = TaskSchema(
    name="configure_batch_async", input_schema=BatchCameraConfigRequest, output_schema=BatchOperationResponse
)
...
set_image_enhancement_task = TaskSchema(
    name="set_image_enhancement", input_schema=ImageEnhancementRequest, output_schema=BoolResponse
)
configure_batch_sync_task = TaskSchema(
    name="configure_batch_sync", input_schema=BatchCameraConfigRequest, output_schema=BatchOperationResponse
)
...
export_batch_config_task = TaskSchema(
    name="export_batch_config", input_schema=BatchConfigExportRequest, output_schema=BatchOperationResponse
)
import_batch_config_task = TaskSchema(
    name="import_batch_config", input_schema=BatchConfigImportRequest, output_schema=BatchOperationResponse
)
...
set_concurrent_limit_task = TaskSchema(
    name="set_concurrent_limit", input_schema=NetworkConcurrentLimitRequest, output_schema=BoolResponse
)
...
class CameraAPIService(Service):
    ...
    def __init__(self, **kwargs):
        ...
        """
        super().__init__(
            summary="Camera API Service", description="REST API for camera management and control", **kwargs
        )
    ...
    def _register_camera_management_endpoints(self):
        ...
        self.add_endpoint(
            "cameras/{camera}/connection", check_camera_connection, check_camera_connection_task, methods=["GET"]
        )
        self.add_endpoint("cameras/{camera}/info", get_camera_info, get_camera_info_task, methods=["GET"])

        # Camera Management - POST endpoints
        self.add_endpoint(
            "cameras/batch/initialize", initialize_cameras_batch, initialize_cameras_batch_task, methods=["POST"]
        )
    ...
    def _register_backend_management_endpoints(self):
        ...
        self.add_endpoint(
            "backends/{backend}/info", get_specific_backend_info, get_specific_backend_info_task, methods=["GET"]
        )
    ...
    def _register_configuration_endpoints(self):
        ...
        self.add_endpoint(
            "cameras/{camera}/exposure/range", get_exposure_range, get_exposure_range_task, methods=["GET"]
        )
        ...
        self.add_endpoint(
            "cameras/{camera}/white-balance/modes",
            get_white_balance_modes,
            get_white_balance_modes_task,
            methods=["GET"],
        )
        ...
        self.add_endpoint(
            "cameras/batch/configure/async", configure_batch_async, configure_batch_async_task, methods=["POST"]
        )
        ...
        self.add_endpoint(
            "cameras/{camera}/pixel-format/modes", get_pixel_formats, get_pixel_formats_task, methods=["GET"]
        )
        self.add_endpoint(
            "cameras/{camera}/image-enhancement", get_image_enhancement, get_image_enhancement_task, methods=["GET"]
        )
        ...
        self.add_endpoint(
            "cameras/{camera}/image-enhancement", set_image_enhancement, set_image_enhancement_task, methods=["PUT"]
        )
        ...
        self.add_endpoint(
            "cameras/batch/configure/sync", configure_batch_sync, configure_batch_sync_task, methods=["POST"]
        )

        # Configuration Persistence - POST endpoints
        self.add_endpoint(
            "cameras/batch/config/export", export_batch_config, export_batch_config_task, methods=["POST"]
        )
        self.add_endpoint(
            "cameras/batch/config/import", import_batch_config, import_batch_config_task, methods=["POST"]
        )
