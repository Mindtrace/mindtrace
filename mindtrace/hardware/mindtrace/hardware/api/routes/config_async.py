...
async def get_exposure(
    ...
    try:
        ...
        return FloatResponse(
            success=True,
            data=exposure,
            message=f"Current exposure: {exposure} μs"
        )
...
async def set_exposure(
    ...
    try:
        ...
        if success:
            ...
            return BoolResponse(
                success=True,
                message=f"Exposure set to {request.exposure} μs"
            )
        else:
            logger.warning(f"Failed to set exposure for '{request.camera}' to {request.exposure} μs")
            raise HTTPException(
                status_code=422,
                detail=f"Failed to set exposure to {request.exposure} μs"
            )
...
async def get_exposure_range(
    ...
    try:
        ...
        raise HTTPException(
            status_code=422,
            detail=f"Failed to get exposure range: {str(e)}"
        )
    except Exception as e:
...
async def get_trigger_mode(
    ...
    try:
        ...
        return StringResponse(
            success=True,
            data=trigger_mode,
            message=f"Current trigger mode: {trigger_mode}"
        )
        ...
        raise HTTPException(
            status_code=422,
            detail=f"Failed to get trigger mode: {str(e)}"
        )
    except Exception as e:
...
async def set_trigger_mode(
    ...
    try:
        ...
        if request.mode not in ["continuous", "trigger"]:
            raise HTTPException(
                status_code=400,
                detail="Invalid trigger mode. Must be 'continuous' or 'trigger'"
            )
        ...
        if success:
            ...
            return BoolResponse(
                success=True,
                message=f"Trigger mode set to '{request.mode}'"
            )
        else:
            logger.warning(f"Failed to set trigger mode for '{request.camera}' to '{request.mode}'")
            raise HTTPException(
                status_code=422,
                detail=f"Failed to set trigger mode to '{request.mode}'"
            )
...
async def get_white_balance(
    ...
    try:
        ...
        return StringResponse(
            success=True,
            data=white_balance,
            message=f"Current white balance: {white_balance}"
        )
        ...
        raise HTTPException(
            status_code=422,
            detail=f"Failed to get white balance: {str(e)}"
        )
    except Exception as e:
...
async def set_white_balance(
    ...
    try:
        ...
        if success:
            ...
            return BoolResponse(
                success=True,
                message=f"White balance set to '{request.mode}'"
            )
        else:
            logger.warning(f"Failed to set white balance for '{request.camera}' to '{request.mode}'")
            raise HTTPException(
                status_code=422,
                detail=f"Failed to set white balance to '{request.mode}'"
            )
...
async def get_white_balance_modes(
    ...
    try:
        ...
        return WhiteBalanceListResponse(
            success=True,
            data=wb_modes,
            message=f"Found {len(wb_modes)} available white balance modes"
        )
        ...
        raise HTTPException(
            status_code=422,
            detail=f"Failed to get white balance modes: {str(e)}"
        )
    except Exception as e:
...
async def configure_batch_async(
    ...
    **Supported Parameters:**
    ...
    try:
        ...
        logger.info(
            f"Batch async configuration completed: {successful_count} successful, {failed_count} failed"
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Batch async configuration error: {str(e)}"
        ) 