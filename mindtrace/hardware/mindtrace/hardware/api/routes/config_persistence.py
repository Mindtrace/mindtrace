...
async def export_batch_config(
    ...
    try:
        ...
        logger.info(
            f"Batch export completed: {successful_count} successful, {failed_count} failed"
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Batch export error: {str(e)}"
        )
...
async def import_batch_config(
    ...
    try:
        ...
        logger.info(
            f"Batch import completed: {successful_count} successful, {failed_count} failed"
        )
