...
async def get_bandwidth_info(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve network bandwidth info: {str(e)}"
        )
...
async def get_concurrent_limit(
    ...
    try:
        ...
        return IntResponse(
            success=True,
            data=current_limit,
            message=f"Current concurrent capture limit: {current_limit}"
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve concurrent capture limit: {str(e)}"
        )


@router.post("/concurrent-limit", response_model=BoolResponse)
async def set_concurrent_limit(
    request: NetworkConcurrentLimitRequest,
    manager: CameraManager = Depends(get_camera_manager)
) -> BoolResponse:
    """
    ...
    **Recommendations:**
    ...
    try:
        ...
        return BoolResponse(
            success=True,
            data=True,
            message=f"Concurrent capture limit set to {request.limit}"
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to set concurrent capture limit: {str(e)}"
        )
...
async def get_network_health(
    ...
    try:
        ...
        return DictResponse(
            success=True,
            data=health_report,
            message=f"Network health: {health_status} ({usage_percentage:.1f}% usage)"
        )
