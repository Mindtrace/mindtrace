...
async def list_active_cameras(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to list active cameras: {str(e)}"
        )
...
async def initialize_camera(
    ...
    try:
        ...
        if request.camera in active_cameras:
            return BoolResponse(
                success=True,
                message=f"Camera '{request.camera}' is already initialized"
            )
        ...
        return BoolResponse(
            success=True,
            message=f"Camera '{request.camera}' initialized successfully"
        )
        ...
        raise HTTPException(
            status_code=422,
            detail=f"Camera initialization failed: {str(e)}"
        )
    except ValueError:
        # Re-raise ValueError to let app.py handlers convert to 400
        raise
...
async def initialize_cameras_batch(
    ...
    try:
        ...
        failed_cameras = await manager.initialize_cameras(
            request.cameras,
        ...
        logger.info(
            f"Batch initialization completed: {successful_count} successful, {failed_count} failed"
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Batch initialization failed: {str(e)}"
        )
...
async def close_camera(
    ...
    try:
        ...
        return BoolResponse(
            success=True,
            message=f"Camera '{validated_camera}' closed successfully"
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to close camera: {str(e)}"
        )
...
async def close_all_cameras(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to close all cameras: {str(e)}"
        )
...
async def get_camera_info(
    ...
    try:
        ...
        return StatusResponse(
            success=True,
            data=info,
            message=f"Camera information retrieved for '{validated_camera}'"
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get camera information: {str(e)}"
        ) 