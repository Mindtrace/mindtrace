...
async def get_gain(
    ...
    try:
        ...
        raise HTTPException(
            status_code=422,
            detail=f"Failed to get gain: {str(e)}"
        )
...
async def set_gain(
    ...
    try:
        ...
        if ":" not in request.camera:
            raise HTTPException(
                status_code=400,
                detail="Invalid camera name format. Expected 'Backend:device_name'"
            )
        ...
        if request.camera not in active_cameras:
            raise HTTPException(
                status_code=404,
                detail=f"Camera '{request.camera}' is not initialized"
            )
        ...
        if success:
            ...
            return BoolResponse(
                success=True,
                message=f"Gain set to {request.gain}"
            )
        else:
            logger.warning(f"Failed to set gain for '{request.camera}' to {request.gain}")
            raise HTTPException(
                status_code=422,
                detail=f"Failed to set gain to {request.gain}"
            )
...
async def get_gain_range(
    ...
    try:
        ...
        raise HTTPException(
            status_code=422,
            detail=f"Failed to get gain range: {str(e)}"
        )
    except Exception as e:
...
async def get_roi(
    ...
    try:
        ...
        raise HTTPException(
            status_code=422,
            detail=f"Failed to get ROI: {str(e)}"
        )
    except Exception as e:
...
async def set_roi(
    ...
    try:
        ...
        if request.width <= 0 or request.height <= 0:
            raise HTTPException(
                status_code=400,
                detail="ROI width and height must be positive"
            )
        
        if request.x < 0 or request.y < 0:
            raise HTTPException(
                status_code=400,
                detail="ROI x and y coordinates must be non-negative"
            )
        ...
        if success:
            logger.info(
                f"Set ROI for '{request.camera}' to "
                f"({request.x}, {request.y}, {request.width}, {request.height})"
            )
            return BoolResponse(
                success=True,
                message=f"ROI set to ({request.x}, {request.y}, {request.width}, {request.height})"
            )
        else:
...
async def reset_roi(
    ...
    try:
        ...
        if success:
            ...
            return BoolResponse(
                success=True,
                message="ROI reset to full sensor size"
            )
        else:
            logger.warning(f"Failed to reset ROI for '{validated_camera}'")
            raise HTTPException(
                status_code=422,
                detail="Failed to reset ROI"
            )
        ...
        raise HTTPException(
            status_code=422,
            detail=f"Failed to reset ROI: {str(e)}"
        )
    except Exception as e:
...
async def get_pixel_format(
    ...
    try:
        ...
        return StringResponse(
            success=True,
            data=pixel_format,
            message=f"Current pixel format: {pixel_format}"
        )
        ...
        raise HTTPException(
            status_code=422,
            detail=f"Failed to get pixel format: {str(e)}"
        )
    except Exception as e:
...
async def set_pixel_format(
    ...
    try:
        ...
        if success:
            ...
            return BoolResponse(
                success=True,
                message=f"Pixel format set to '{request.format}'"
            )
        else:
            logger.warning(f"Failed to set pixel format for '{request.camera}' to '{request.format}'")
            raise HTTPException(
                status_code=422,
                detail=f"Failed to set pixel format to '{request.format}'"
            )
...
async def get_pixel_format_options(
    ...
    try:
        ...
        return PixelFormatListResponse(
            success=True,
            data=pixel_formats,
            message=f"Found {len(pixel_formats)} available pixel formats"
        )
        ...
        raise HTTPException(
            status_code=422,
            detail=f"Failed to get pixel formats: {str(e)}"
        )
    except Exception as e:
...
async def get_image_enhancement(
    ...
    try:
        ...
        raise HTTPException(
            status_code=422,
            detail=f"Failed to get image enhancement: {str(e)}"
        )
    except Exception as e:
...
async def set_image_enhancement(
    ...
    try:
        ...
        if success:
            ...
            return BoolResponse(
                success=True,
                message=f"Image enhancement {status}"
            )
        else:
            logger.warning(f"Failed to set image enhancement for '{request.camera}'")
            raise HTTPException(
                status_code=422,
                detail="Failed to set image enhancement"
            )
...
async def configure_batch_sync(
    request: BatchCameraConfigRequest,
    manager: CameraManager = Depends(get_camera_manager)
) -> BatchOperationResponse:
    """
    ...
    **Supported Parameters:**
    ...
    try:
        ...
        logger.info(
            f"Batch sync configuration completed: {successful_count} successful, {failed_count} failed"
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Batch sync configuration error: {str(e)}"
        ) 