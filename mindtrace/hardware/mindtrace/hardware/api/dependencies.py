    ...
    async def get_manager(self) -> CameraManager:
        ...
        if self._manager is None:
            try:
                ...
                raise HTTPException(
                    status_code=503,
                    detail=f"Camera system unavailable: {str(e)}"
                )
...
async def get_camera_manager_with_validation() -> CameraManager:
    ...
    try:
        ...
        if not backends:
            ...
            raise HTTPException(
                status_code=503,
                detail="No camera backends available"
            )
        ...
        raise HTTPException(
            status_code=503,
            detail=f"Camera system validation failed: {str(e)}"
        )
...
async def validate_camera_exists(
    camera: str = Depends(validate_camera_name),
