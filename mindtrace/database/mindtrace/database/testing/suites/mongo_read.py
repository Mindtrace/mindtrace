"""Mongo ODM read throughput benchmark."""
...
class DatabaseMongoReadInput(BaseModel):
...
class DatabaseMongoReadCeilingSuite(BenchTestSuite):
    suite_id = "database.stress.mongo_read_ceiling"
    title = "Database stress — Mongo read ceiling"
    description = "Pre-seeds Mongo ODM benchmark documents and repeatedly reads them by ID or filter."
    tags = frozenset({"stress", "database", "mongo"})
    requires = ("mongo",)
    task_schema = TaskSchema(name=suite_id, input_schema=DatabaseMongoReadInput, output_schema=BenchResultSchema)
    resource_schema = DatabaseMongoReadResources
...
async def _run_async(config: BenchSuiteConfig, reporter: BenchReporter) -> BenchResult:
    ...
    read_mode = str(config.parameters.get("read_mode", "get_by_id"))
    ...
    try:
        ...
        while time.perf_counter() < deadline and not reporter.is_cancelled():
            ...
            try:
                if read_mode == "find_filter":
                    rows = await odm.find({"run_id": run_id, "shard": index % 16})
                    if not rows:
                        raise LookupError("find_filter returned no rows")
                else:
                    doc_id = rng.choice(ids) if read_pattern == "random" else ids[index % len(ids)]
                    await odm.get(doc_id)
